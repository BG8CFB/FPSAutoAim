...
class MyLoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32,cvt_color='BGR'):
        # for img in path:
        #     if type(img) != np.ndarray or len(img.shape) != 3:
        #         raise TypeError('There is an object in source that is not a picture read by cv2')
        # Padded resize
        img = letterbox(path, img_size, stride=stride)[0]
        # Convert
